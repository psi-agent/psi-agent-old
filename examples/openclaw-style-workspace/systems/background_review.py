...
_FLOW_REVIEW_PROMPT = (
    "Review the conversation above. The agent just completed a tool-heavy turn "
    "(10+ tool calls). Decide whether the actions taken could be usefully captured "
    "as a reusable flow.\n\n"
    "A flow is worth capturing when:\n"
    "  • A clear multi-step pipeline was executed (research → synthesise → write, "
    "or fetch → transform → store, etc.)\n"
    "  • The same sequence would likely be valuable in future sessions.\n"
    "  • The steps are concrete enough to codify as a TypeScript flow.\n\n"
    "If a flow is worth capturing, use the flow_manage tool to create it in "
    "flows/curated/. The FLOW.md must include:\n"
    "  - YAML frontmatter (name, description, category, created_by: agent, created_at)\n"
    "  - A description section explaining what the flow does and when to use it\n"
    "  - A ```typescript code block with the flow.ts implementation\n\n"
    "If nothing is worth capturing as a flow, say 'Nothing to capture.' and stop."
)
...
class BackgroundReview:
    ...
    async def maybe_spawn(
        ...
        do_flows = tool_call_count >= self.SKILL_TOOL_THRESHOLD
        ...
        # Flow review — spawned independently whenever tool threshold is hit
        if do_flows:
            asyncio.create_task(
                self._run_review(copy.deepcopy(messages_snapshot), _FLOW_REVIEW_PROMPT, {"flow_manage"}),
                name=f"background-review-flow-turn{self._turn_count}",
            )
