...
FLOWS_HEADER = """\
## Flows
Scan <available_flows>. If one matches the current task, use `flow_manage action=view` to read it and follow the flow.ts implementation pattern.
If none match, generate a new flow.ts and save it to flows/adhoc/<name>/flow.ts using the bash or write tool.\
"""
